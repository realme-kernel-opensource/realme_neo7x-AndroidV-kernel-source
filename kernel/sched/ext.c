...
struct work_struct scx_err_exit_work;
...
u32 SCX_BPF_DSQS_DEADLINE[MAX_GLOBAL_DSQS] = {4, 4, 4, 4, 4, 8, 8, 16, 24, 100};
...
/*
 * hmbird_sched stats: record critical events for debuging.
 * disabled default.
 * trigger record: echo 1 > HMBIRD_PROC_DIR/slim_stats
 * read stats msg: cat HMBIRD_PROC_DIR/hmbird_stats
 *
 * there are two parts:regular stats & misc stats.
 * regular stats:u64 (*)[2]
 * misc stats:pcp stats, per dsq stats, ...
 *
 * If you want to add stat item, for regular stats, add item in:
 * {enum stat_items} -- change DWORD_STAT_END if appending to the tail.
 * {stats_str[MAX_ITEMS]}
 * {struct stats_struct}
 *
 * If add misc stats, also add code for:
 * {slim_stats_record}
 * {stats_print}
*/
enum stat_items {
...
static u64 get_cpus_max_util(struct cpumask *mask, bool logout)
{
	int cpu;
	u64 max = 0;
	u64 util, ratio;
	...
	for_each_cpu(cpu, mask) {
		...
		if (logout)
			scx_internal_systrace("C|9999|Cpu%d_uratio|%llu\n", cpu, ratio);
                if (ratio > max)
                        max = ratio;
        }
	return max;
...
{
	if (mask == iso_masks.big) {
		if (!parctrl_high_ratio && !parctrl_low_ratio)
			return true;
	} else if (mask == iso_masks.little) {
		if (!parctrl_high_ratio_l && !parctrl_low_ratio_l)
			return true;
	} else {}

	return get_cpus_max_util(mask, false) > hres;
}

static void util_thres_for_cluster(struct cpumask *mask, u64 *max,
				   int high, int low, bool *over, bool *under)
{
	if (high || low) {
		*max = get_cpus_max_util(mask, true);
		*over = *max > high;
		*under = *max < low;
	} else {
		*max = 0;
...
{
	u64 lmax = 0, bmax = 0, tmax = 0;
	bool l_over, l_under, b_over, b_under, t_over, t_under;
	...
	util_thres_for_cluster(iso_masks.little, &lmax,
			parctrl_high_ratio_l, parctrl_low_ratio_l, &l_over, &l_under);

	util_thres_for_cluster(iso_masks.big, &bmax,
			parctrl_high_ratio, parctrl_low_ratio, &b_over, &b_under);
	...
	if (is_partial_enabled()) {
		util_thres_for_cluster(iso_masks.partial, &tmax,
			isoctrl_high_ratio, isoctrl_low_ratio, &t_over, &t_under);

		if (!is_iso_par_free() && t_over) {
			scx_info_trace("<par>partial max = %llu\n", tmax);
			free_isocpu(true);
		}
		else if (is_iso_par_free() && t_under)
			free_isocpu(false);
...
{
	scx_internal_systrace("C|9999|Cpu%d_dsq_id|%d\n", cpu, idx);
}
...
static int consume_timeout_dsq(struct rq *rq, struct rq_flags *rf, struct cluster_ctx *ctx, bool period)
{
	int i;
	bool is_timeout;

	if (!period && consume_pcp_dsq(rq, rf, false))
		return 1;

	for(i = ctx->lower; i < ctx->upper; i++) {
		/*
		 * No lock here, it's very hot path, locking operation may bring many lock contention,
		 * gdsqs[i].is_timeout may change during, let it be...
		 */
		...
		if (is_timeout) {
			 ...
			 */
				...
				scx_info_trace("<timeout>cpu[%d] consume a timeout gdsq[%d] task,"
						"lrescue = %d,  brescue = %d\n", cpu_of(rq), i,
						is_little_need_rescue(), is_big_need_rescue());
...
{
	struct cluster_ctx ctx = {.lower = GDSQ_DSQ_START_IDX, .upper = NON_PERIOD_START};
        int i;

	if (consume_timeout_dsq(rq, rf, &ctx, true))
		return 1;
...
static int gen_cluster_ctx(struct cluster_ctx* ctx, enum cpu_type type, bool for_timeout)
{
	/* If any dsq timeout, any cpu can consume that dsq. */
	if (for_timeout) {
...
static int consume_non_period_dsq(struct rq *rq, struct rq_flags *rf, enum cpu_type type)
{
	...
	if (gen_cluster_ctx(&ctx, type, true))
		return 0;

	if (consume_timeout_dsq(rq, rf, &ctx, false))
		return 1;

	if (gen_cluster_ctx(&ctx, type, false))
		return 0;
...
void scan_timeout(struct rq *rq)
{
	...
	for (i = GDSQ_DSQ_START_IDX; i < NON_PERIOD_END; i++) {
		dsq = &gdsqs[i];
...
static void init_child_tg(struct cgroup *cgrp, struct task_group *tg)
{
	...
	if (NULL == l1cgrp) {
		scx_err("<error> : can't find l1 parent cgrp\n");
		tg->scx_deadline_idx = DEFAULT_CGROUP_DL_IDX;
		return;
	}
...
static void yield_task_scx(struct rq *rq)
{
	...
	if (is_isolate_task(p))
		slim_stats_record(YIELD_CNT, 0, 0, 0);
...
static int check_misfit_task_on_little(struct task_struct *p, struct rq *rq, struct scx_dispatch_q *dsq)
{
	...
	if (gen_cluster_ctx(&ctx, BIG, false))
		return false;
	...
	task_util = uclamp_rq_util_with(rq, task_util, p);
 ...
 * we can not output to muny logs to systrace buffer even in debug mode
...
static void systrace_output_cpu_ds(struct rq *rq, struct task_struct *p)
{
	...
	if (util >= misfit_ds) {
		scx_internal_systrace("C|9999|cpu_%d_ds|%llu\n", cpu, util);
		per_cpu(is_last_exceed, cpu) = true;
	}
	else if (per_cpu(is_last_exceed, cpu) && (util < misfit_ds)) {
		scx_internal_systrace("C|9999|cpu_%d_ds|%d\n", cpu, 0);
...
static int heavy_rt_placement(struct task_struct *p, int prev)
{
	struct cpumask tmp = {.bits = {0}};
	int cpu;
	u64 util, min_uclam, max_clamp;

        trace_android_vh_get_util(-1, p, &util);
	min_uclam = uclamp_eff_value(p, UCLAMP_MIN);
	max_clamp = uclamp_eff_value(p, UCLAMP_MAX);
	util = clamp(util, min_uclam, max_clamp);
...
static void inform_ext_onoff_from_systrace(void)
{
	...
	scx_output_systrace("C|9999|scx_status|%d\n", curr_ss);
	scx_internal_systrace("C|9999|partial_enable|%d\n", is_partial_enabled());
	scx_internal_systrace("C|9999|l_need_rescue|%d\n", is_little_need_rescue());
	scx_internal_systrace("C|9999|b_need_rescue|%d\n", is_big_need_rescue());
	scx_internal_systrace("C|9999|free_iso|%d\n", is_iso_par_free());
...
static void scx_switch_log(enum switch_stat ss, bool finish, bool enable)
{
	...
	if (ss == SCX_DISABLED || ss == SCX_ENABLED)
		scx_debug("<switch>: ext %s %s at jiffies = %lu, clock = %lu\n",
...
static void scx_ops_disable_workfn(struct kthread_work *work)
{
	...
	case SCX_OPS_DISABLED:
		WARN_ON_ONCE(scx_ops_set_enable_state(SCX_OPS_DISABLED) !=
			     SCX_OPS_DISABLING);
		scx_switch_log(SCX_DISABLED, 0, 0);
...
void ext_ctrl(bool enable)
{
	if (!atomic_read(&ext_module_loaded)) {
		scx_err("<init>:ext module not loaded, failed to enable ext!\n");
		return;
	}
...
void __init init_sched_ext_class(void)
{
	...
	if (init_task.scx) {
		...
		scx_err("<fatal>:alloc init_task.scx failed!!!\n");
	}
	...
	for_each_possible_cpu(cpu) {
		...
		if (rq->scx)
			...
			scx_err("<fatal>:alloc rq->scx failed!!!\n");
